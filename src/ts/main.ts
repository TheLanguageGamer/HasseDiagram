function enableProperty(name : string, value : boolean) {
...
class Graph {
	...
	transitiveclosure : NodeRelation[][] = [];
	joins : NodeRelation[][] = [];
	meets : NodeRelation[][] = [];

	isCyclic : boolean = false;
	hasMeets : boolean = false;
	hasJoins : boolean = false;
	isTotalOrder : boolean = false;

	readonly UNDEFINED : number = -2;
	readonly DISABLED : number = -1;

	constructor() {

	}

	expandMatrix(matrix : NodeRelation[][], N : number) {
	...
	transitiveClosureAuxilary(i : number, j : number) {
...
class SetBox implements Component {

	layout : Layout;
	children : Component[] = [];
	onChange : (elements : SetElement[], edges : Edge[]) => void;
...
class Main {
	...
	constructor(container : HTMLElement) {
		...
		let setBoxPLayout = new Layout(
			0, 0, 20, 50,
			0.5, 0.8, -30, -50
		);
		let setBoxP = new SetBox(
			setBoxPLayout,
			"P",
			function(elements : SetElement[], edges : Edge[]) {
				graphP.update(elements, edges, "P");
			}
		);

		let graphQ = new Graph();
		let setBoxQLayout = new Layout(
			0.5, 0, 10, 50,
			0.5, 0.8, -30, -50
		);
		let setBoxQ = new SetBox(
			setBoxQLayout,
			"Q",
			function(elements : SetElement[], edges : Edge[]) {
